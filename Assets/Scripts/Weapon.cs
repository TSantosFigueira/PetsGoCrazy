...
public class Weapon : MonoBehaviour {
	...
	void Update () {
        ...
        if(timer < 1) anim.SetBool("Shoot", false);
        ...
        if (Input.GetButtonDown(shootButton) && timer <= 0f)
        {
            ...
            {
                GameObject bullet = Instantiate(projectile, transform.position + new Vector3(1.1f, 0), Quaternion.Euler(Vector3.zero));
                bullet.GetComponent<Rigidbody2D>().AddForce(new Vector2(velocity, 0), ForceMode2D.Impulse);
            }
            else
            {
                GameObject bullet = Instantiate(projectile, transform.position + new Vector3(-1.1f, 0), Quaternion.Euler(new Vector2(0, 180)));
                bullet.GetComponent<Rigidbody2D>().AddForce(new Vector2(-velocity, 0), ForceMode2D.Impulse);
            }
        }
	}
